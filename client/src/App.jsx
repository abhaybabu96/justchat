...
import { saveMessages, loadMessages, clearMessages, isSaved, getTimeLeft } from './storage';
import './index.css';
...
export default function App() {
  ...
  const joinRoom = () => {
    ...
    if (code.length < 3) { setError('Enter a valid room code'); return; }
    socket.connect();
  ...
  const handleTyping = (e) => {
    ...
    typingTimer.current = setTimeout(() =>
      socket.emit('stop-typing', roomCode), 1500);
  };
...
// ── Home Screen ───────────────────────────────────────────
function HomeScreen({ myCode, onStart, onJoin, joinInput, setJoinInput, error, setError }) {
  return (
      ...
      <div style={{ width: '100%', maxWidth: 400,
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
          ...
          <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)',
            color: 'var(--accent)', letterSpacing: .5 }}>justchat</span>
        </div>

        {/* Heading */}
        <h1 style={{ fontSize: 26, fontWeight: 300, color: 'var(--text)',
          lineHeight: 1.25, marginBottom: 10 }}>
          Simple, private chats.<br />
        ...
        <p style={{ fontSize: 14, color: 'var(--text2)', lineHeight: 1.65,
          ...
          Your conversation belongs only to you — no accounts, no servers storing
          your words, no history. The moment you refresh, it's gone.
        </p>

        {/* Code card */}
        <div style={{ background: 'var(--bg2)', border: '1.5px solid var(--border2)',
          borderRadius: 14, padding: 22, marginBottom: 12,
          position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: 0, right: 0, width: 80,
            height: 80, background: 'var(--accent-bg)',
            borderRadius: '0 14px 0 80px', opacity: .5 }} />
          ...
          <div style={{ fontSize: 30, fontFamily: 'var(--font-mono)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6,
            ...
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
              <path d="M2 6.5L5 9.5L11 3.5" stroke="#16a34a"
        ...
        <div style={{ display: 'flex', alignItems: 'center',
          gap: 10, margin: '18px 0 14px' }}>
          <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
          <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)',
            color: 'var(--text3)', letterSpacing: .5 }}>or enter a code</span>
          <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
        </div>
        ...
        <div style={{ marginTop: 20, padding: '14px 16px',
          ...
          <div style={{ fontSize: 12, color: '#92400e', lineHeight: 1.65 }}>
            No one else can see your messages — not us, not anyone. Only the
            two people in this room, and only while both are connected.
          </div>
        </div>
        ...
        <div style={{ marginTop: 20, display: 'flex',
          ...
          {['No accounts', 'No logs', 'No history'].map((t, i) => (
            <span key={i} style={{ fontSize: 11,
              ...
              {i > 0 && <span style={{ marginRight: 16,
                color: 'var(--border2)' }}>·</span>}{t}
            </span>
          ))}
        </div>
      </div>
...
// ── Chat Screen ───────────────────────────────────────────
function ChatScreen({ roomCode, messages, input, isTyping, onlineCount,
  onInputChange, onSend, onLeave, bottomRef, saved, onSave, onClearSave }) {
  ...
  useEffect(() => {
    if (!saved) { setTimeLeft(null); return; }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column',
      maxWidth: 560, margin: '0 auto', background: 'var(--bg)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 15,
              fontWeight: 600, letterSpacing: 4, color: 'var(--text)' }}>
              {roomCode}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5,
            ...
            <div style={{ width: 6, height: 6, borderRadius: '50%',
              ...
              animation: connected ? 'pulse 2s infinite' : 'shimmer 1.8s infinite' }} />
            <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
              ...
              {timeLeft && (
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)',
                  color: 'var(--text3)', paddingRight: 2 }}>
                  clears in {timeLeft}
                </span>
              )}
            </div>
      ...
      {/* Saved notice banner */}
      {saved && (
        <div style={{ padding: '8px 18px', background: 'var(--green-bg)',
          borderBottom: '1px solid var(--green-bdr)',
          display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            <path d="M1.5 6L4.5 9L10.5 3" stroke="#16a34a"
              strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 16px',
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex',
            ...
            <div style={{
              maxWidth: '72%', padding: '10px 15px', borderRadius: 16,
              borderBottomRightRadius: msg.sender === 'me' ? 3 : 16,
              borderBottomLeftRadius: msg.sender === 'them' ? 3 : 16,
              background: msg.sender === 'me' ? 'var(--me-bg)' : 'var(--them-bg)',
              color: msg.sender === 'me' ? 'var(--me-text)' : 'var(--them-text)',
        ...
        {isTyping && (
          <div style={{ display: 'flex', justifyContent: 'flex-start',
            animation: 'fadeUp .2s ease both' }}>
            <div style={{ padding: '11px 15px', borderRadius: 16,
              borderBottomLeftRadius: 3, background: 'var(--them-bg)',
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)',
        background: 'var(--bg2)', display: 'flex',
        gap: 10, alignItems: 'center', flexShrink: 0 }}>
        <input
          value={input}
          onChange={onInputChange}
          onKeyDown={(e) => e.key === 'Enter' && onSend()}
          placeholder={connected ? 'Message...' : 'Waiting for the other person...'}
          disabled={!connected}
          autoFocus
          style={{ flex: 1, background: 'var(--bg)',
            border: '1.5px solid var(--border2)', borderRadius: 24,
            padding: '11px 18px', fontSize: 14, color: 'var(--text)',
            outline: 'none', opacity: connected ? 1 : .5,
            cursor: connected ? 'text' : 'not-allowed' }}
        />
        <button onClick={onSend}
          disabled={!connected}
          style={{ width: 40, height: 40, borderRadius: '50%',
            border: 'none', background: 'var(--text)', color: 'var(--bg2)',
            fontSize: 16, cursor: connected ? 'pointer' : 'not-allowed',
            flexShrink: 0, display: 'flex',
            alignItems: 'center', justifyContent: 'center',
            opacity: connected ? 1 : .4 }}>
          ↑
        </button>
      </div>
    </div>
